...
const ALLOWED_CLIENT_EVENTS: ReadonlySet<string> = new Set<ClientEvent['type']>([
  ...
  'investigation.getLatestReplanRecommendation',
  'investigation.execute',
  'investigation.pauseTask',
  'investigation.resumeTask',
  'investigation.reprioritizeTask',
  'investigation.createTask',
  'investigation.applyHumanInterruption',
  'investigation.cancelTask',
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  investigation: {
    create: (payload: import('../shared/cyber/investigation-types').CreateInvestigationInput) =>
      invoke({ type: 'investigation.create', payload }),
    list: (): Promise<import('../shared/cyber/investigation-types').Investigation[]> =>
      invoke({ type: 'investigation.list', payload: {} }),
    get: (investigationId: string): Promise<import('../shared/cyber/investigation-types').Investigation | null> =>
      invoke({ type: 'investigation.get', payload: { investigationId } }),
    open: (investigationId: string): Promise<import('../shared/cyber/investigation-types').Investigation> =>
      invoke({ type: 'investigation.open', payload: { investigationId } }),
    resume: (investigationId: string): Promise<import('../shared/cyber/investigation-types').Investigation> =>
      invoke({ type: 'investigation.resume', payload: { investigationId } }),
    archive: (investigationId: string): Promise<import('../shared/cyber/investigation-types').Investigation> =>
      invoke({ type: 'investigation.archive', payload: { investigationId } }),
    plan: (investigationId: string): Promise<import('../main/investigation/parallel-investigation-engine').InvestigationPlan> =>
      invoke({ type: 'investigation.plan', payload: { investigationId } }),
    replan: (investigationId: string): Promise<import('../main/investigation/parallel-investigation-engine').InvestigationPlan> =>
      invoke({ type: 'investigation.replan', payload: { investigationId } }),
    getLatestReplanRecommendation: (
      investigationId: string
    ): Promise<import('../shared/cyber/investigation-types').InvestigationEvent | null> =>
      invoke({ type: 'investigation.getLatestReplanRecommendation', payload: { investigationId } }),
    execute: (investigationId: string): Promise<{ investigationId: string; plan: import('../main/investigation/parallel-investigation-engine').InvestigationPlan }> =>
      invoke({ type: 'investigation.execute', payload: { investigationId } }),
    pauseTask: (plannedTaskId: string, reason?: string): Promise<{ success: boolean }> =>
      invoke({ type: 'investigation.pauseTask', payload: { plannedTaskId, reason } }),
    resumeTask: (plannedTaskId: string): Promise<{ success: boolean }> =>
      invoke({ type: 'investigation.resumeTask', payload: { plannedTaskId } }),
    reprioritizeTask: (plannedTaskId: string, priority: number, rationale?: string): Promise<{ success: boolean }> =>
      invoke({ type: 'investigation.reprioritizeTask', payload: { plannedTaskId, priority, rationale } }),
    createTask: (
      investigationId: string,
      task: import('../main/investigation/parallel-investigation-engine').PlannedInvestigationTask
    ): Promise<{ success: boolean }> => invoke({ type: 'investigation.createTask', payload: { investigationId, task } }),
    applyHumanInterruption: (
      payload: Extract<ClientEvent, { type: 'investigation.applyHumanInterruption' }>['payload']
    ): Promise<{
      investigationId: string;
      updatedInvestigation: import('../shared/cyber/investigation-types').Investigation | null;
      operationalSummary: string;
    }> => invoke({ type: 'investigation.applyHumanInterruption', payload }),
    cancelTask: (plannedTaskId: string, reason?: string): Promise<{ success: boolean }> =>
      invoke({ type: 'investigation.cancelTask', payload: { plannedTaskId, reason } }),
    redirectTask: (
      plannedTaskId: string,
      updates: { description?: string; role?: import('../main/investigation/parallel-investigation-engine').InvestigationAgentRole }
    ): Promise<{ success: boolean }> =>
      invoke({ type: 'investigation.redirectTask', payload: { plannedTaskId, ...updates } }),
    exportReport: (investigationId: string): Promise<{ success: boolean; path?: string; error?: string }> =>
      invoke({ type: 'investigation.exportReport', payload: { investigationId } }),
    getReport: (
      investigationId: string
  ...
  interface Window {
    electronAPI: {
      ...
      investigation: {
        ...
        archive: (investigationId: string) => Promise<import('../shared/cyber/investigation-types').Investigation>;
        plan: (investigationId: string) => Promise<import('../main/investigation/parallel-investigation-engine').InvestigationPlan>;
        execute: (investigationId: string) => Promise<{
          investigationId: string;
          plan: import('../main/investigation/parallel-investigation-engine').InvestigationPlan;
        }>;
        cancelTask: (plannedTaskId: string, reason?: string) => Promise<{ success: boolean }>;
        redirectTask: (
          plannedTaskId: string,
          updates: {
            description?: string;
            role?: import('../main/investigation/parallel-investigation-engine').InvestigationAgentRole;
          }
        ) => Promise<{ success: boolean }>;
        exportReport: (investigationId: string) => Promise<{ success: boolean; path?: string; error?: string }>;
        getReport: (
