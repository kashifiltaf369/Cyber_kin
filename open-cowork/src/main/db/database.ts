...
export interface DatabaseInstance {
  ...
  investigations: {
    create: (investigation: InvestigationRow) => void;
    update: (id: string, updates: Partial<InvestigationRow>) => void;
    get: (id: string) => InvestigationRow | undefined;
    getAll: () => InvestigationRow[];
    delete: (id: string) => void;
  };

  investigationEvents: {
    create: (event: InvestigationEventRow) => void;
    getByInvestigationId: (investigationId: string) => InvestigationEventRow[];
    deleteByInvestigationId: (investigationId: string) => void;
  };

  investigationSessionLinks: {
    link: (investigationId: string, sessionId: string, role: string) => void;
    getInvestigationIdBySessionId: (sessionId: string) => string | null;
    getSessionLinksByInvestigationId: (investigationId: string) => InvestigationSessionLinkRow[];
...
export interface InvestigationRow {
  ...
  human_capability_context: string;
  hypotheses: string;
  evidence: string;
  entities: string;
  graph: string;
...
export interface InvestigationSessionLinkRow {
  investigation_id: string;
  session_id: string;
  role: string;
  created_at: number;
...
function initializeSchema(database: Database.Database): void {
  ...
  `);
    ...
    CREATE TABLE IF NOT EXISTS investigations (
      ...
      human_capability_context TEXT NOT NULL DEFAULT '{}',
      hypotheses TEXT NOT NULL DEFAULT '[]',
      evidence TEXT NOT NULL DEFAULT '[]',
      entities TEXT NOT NULL DEFAULT '[]',
      graph TEXT NOT NULL DEFAULT '{"relationships":[]}',
      timeline TEXT NOT NULL DEFAULT '[]',
      open_questions TEXT NOT NULL DEFAULT '[]',
      ai_tasks TEXT NOT NULL DEFAULT '[]',
      human_tasks TEXT NOT NULL DEFAULT '[]',
  ...
  `);
    ...
    database.exec(`
    CREATE INDEX IF NOT EXISTS idx_investigation_events_investigation_id
    ON investigation_events(investigation_id, created_at)
  `);

    database.exec(`
    CREATE TABLE IF NOT EXISTS investigation_session_links (
      investigation_id TEXT NOT NULL,
  ...
  `);
    ...
    database.exec(`
    CREATE INDEX IF NOT EXISTS idx_investigation_session_links_investigation_id
    ON investigation_session_links(investigation_id, created_at)
  `);
...
export function initDatabase(options: { dbPath?: string } = {}): DatabaseInstance {
  ...
  const insertInvestigation = rawDb.prepare(`
    INSERT OR REPLACE INTO investigations
    (id, title, objective, status, created_at, updated_at, human_context, human_capability_context, hypotheses, evidence, entities, graph, timeline, open_questions, ai_tasks, human_tasks, decisions, conclusions, confidence)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);

  const getInvestigationStmt = rawDb.prepare(`
    SELECT * FROM investigations WHERE id = ?
  `);

  const getAllInvestigationsStmt = rawDb.prepare(`
    SELECT * FROM investigations ORDER BY updated_at DESC
  `);

  const deleteInvestigationStmt = rawDb.prepare(`
    DELETE FROM investigations WHERE id = ?
  `);

  const insertInvestigationEvent = rawDb.prepare(`
    INSERT INTO investigation_events (id, investigation_id, type, actor, summary, data, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);

  const getInvestigationEventsByInvestigationStmt = rawDb.prepare(`
    SELECT * FROM investigation_events WHERE investigation_id = ? ORDER BY created_at ASC
  `);

  const deleteInvestigationEventsByInvestigationStmt = rawDb.prepare(`
    DELETE FROM investigation_events WHERE investigation_id = ?
  `);

  const linkInvestigationSessionStmt = rawDb.prepare(`
    INSERT OR REPLACE INTO investigation_session_links (investigation_id, session_id, role, created_at)
    VALUES (?, ?, ?, ?)
  `);

  const getInvestigationIdBySessionIdStmt = rawDb.prepare(`
    SELECT investigation_id FROM investigation_session_links WHERE session_id = ?
  `);

  const getSessionLinksByInvestigationIdStmt = rawDb.prepare(`
    SELECT * FROM investigation_session_links WHERE investigation_id = ? ORDER BY created_at ASC
  `);

  const deleteInvestigationSessionLinkBySessionIdStmt = rawDb.prepare(`
    DELETE FROM investigation_session_links WHERE session_id = ?
  `);
  ...
  db = {
    ...
    investigations: {
      create: (investigation: InvestigationRow) => {
        insertInvestigation.run(
          ...
          investigation.human_context,
          investigation.human_capability_context,
          ...
          investigation.ai_tasks,
          investigation.human_tasks,
      ...
      update: (id: string, updates: Partial<InvestigationRow>) => {
        ...
        const sql = `UPDATE investigations SET ${setClauses.join(', ')} WHERE id = ?`;
        rawDb.prepare(sql).run(...values);
      },

      get: (id: string): InvestigationRow | undefined => {
        return getInvestigationStmt.get(id) as InvestigationRow | undefined;
      },

      getAll: (): InvestigationRow[] => {
        return getAllInvestigationsStmt.all() as InvestigationRow[];
      },

      delete: (id: string) => {
        deleteInvestigationStmt.run(id);
      },
    },

    investigationEvents: {
      create: (event: InvestigationEventRow) => {
        insertInvestigationEvent.run(
          event.id,
      ...
      getByInvestigationId: (investigationId: string): InvestigationEventRow[] => {
        return getInvestigationEventsByInvestigationStmt.all(investigationId) as InvestigationEventRow[];
      },

      deleteByInvestigationId: (investigationId: string) => {
        deleteInvestigationEventsByInvestigationStmt.run(investigationId);
      },
    },

    investigationSessionLinks: {
      link: (investigationId: string, sessionId: string, role: string) => {
        linkInvestigationSessionStmt.run(investigationId, sessionId, role, Date.now());
      },

      getInvestigationIdBySessionId: (sessionId: string): string | null => {
        const row = getInvestigationIdBySessionIdStmt.get(sessionId) as
          | { investigation_id: string }
          | undefined;
      ...
      getSessionLinksByInvestigationId: (investigationId: string): InvestigationSessionLinkRow[] => {
        return getSessionLinksByInvestigationIdStmt.all(investigationId) as InvestigationSessionLinkRow[];
      },

      deleteBySessionId: (sessionId: string) => {
        deleteInvestigationSessionLinkBySessionIdStmt.run(sessionId);
      },
    },
    ...
    sessions: {
      ...
      delete: (id: string) => {
        deleteInvestigationSessionLinkBySessionIdStmt.run(id);
