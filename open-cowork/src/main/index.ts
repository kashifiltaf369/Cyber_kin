...
function createWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    // Development fallback – assume dev server at localhost:5178
    const fallbackUrl = 'http://localhost:5178';
    void (async () => {
      await waitForDevServer(fallbackUrl, 40, 500);
      if (!mainWindow || mainWindow.isDestroyed()) return;
      try {
        await mainWindow.loadURL(fallbackUrl);
      } catch (error) {
        logError('[App] Failed to load fallback dev server URL:', error);
      }
    })();
    // If in production, load the bundled file
...
app
  ...
  .then(async () => {
    ...
    if (headlessArgs.headless) {
      ...
      investigationService = new InvestigationService(db, headlessSendWithPermission);
      ...
      const headlessExtensionManager = new AgentRuntimeExtensionManager([
        ...
        new InvestigationContextExtension(
          investigationService,
          (sessionId: string) => investigationService?.getInvestigationIdBySessionId(sessionId) || null
        ),
        new InvestigationGraphExtension(
          investigationService,
          (sessionId: string) => investigationService?.getInvestigationIdBySessionId(sessionId) || null
        ),
        new InvestigationHypothesisExtension(
          investigationService,
          (sessionId: string) => investigationService?.getInvestigationIdBySessionId(sessionId) || null
        ),
        new InvestigationReplanExtension(
          investigationService,
          (sessionId: string) => investigationService?.getInvestigationIdBySessionId(sessionId) || null
        ),
        new InvestigationCyberAuditExtension(
          investigationService,
          (sessionId: string) => investigationService?.getInvestigationIdBySessionId(sessionId) || null
        ),
        new CyberCapabilityExtension(cyberCapabilityRegistry, {
          ...
          onAuditRecord: (record: CyberActionAuditRecord) => {
            if (!record.investigationId || !investigationService) return;
...
async function handleClientEvent(event: ClientEvent): Promise<unknown> {
  ...
  switch (event.type) {
    case 'investigation.create': {
      ...
      const investigation = investigationService.create(event.payload);
    ...
    case 'investigation.list': {
      ...
      sendToRenderer({ type: 'investigation.list', payload: { investigations } });
      return investigations;
    }

    case 'investigation.get': {
      if (!investigationService) throw new Error('Investigation service not initialized');
      return investigationService.get(event.payload.investigationId);
    }

    case 'investigation.open': {
      if (!investigationService) throw new Error('Investigation service not initialized');
      const investigation = investigationService.open(event.payload.investigationId);
      sendToRenderer({ type: 'investigation.updated', payload: { investigation } });
      return investigation;
    }
    ...
    case 'investigation.resume': {
      if (!investigationService) throw new Error('Investigation service not initialized');
      const investigation = investigationService.resume(event.payload.investigationId);
      sendToRenderer({ type: 'investigation.updated', payload: { investigation } });
      return investigation;
    }
    ...
    case 'investigation.archive': {
      if (!investigationService) throw new Error('Investigation service not initialized');
      const investigation = investigationService.archive(event.payload.investigationId);
      sendToRenderer({ type: 'investigation.updated', payload: { investigation } });
      sendToRenderer({ type: 'investigation.list', payload: { investigations: investigationService.list() } });
      return investigation;
    }
    ...
    case 'investigation.plan': {
      ...
      const plan = investigationOrchestrator.createPlan(event.payload.investigationId);
      sendToRenderer({
        type: 'investigation.plan',
        payload: { investigationId: event.payload.investigationId, plan },
      });
      return plan;
    }
    ...
    case 'investigation.replan': {
      if (!investigationOrchestrator) throw new Error('Investigation orchestrator not initialized');
      const plan = investigationOrchestrator.createReplan(event.payload.investigationId);
      sendToRenderer({
    ...
    case 'investigation.getLatestReplanRecommendation': {
      if (!investigationService) throw new Error('Investigation service not initialized');
      const recommendation = investigationService.getLatestReplanRecommendation(event.payload.investigationId);
      sendToRenderer({
        type: 'investigation.replanRecommendation',
        payload: { investigationId: event.payload.investigationId, recommendation },
      });
      return recommendation;
    }

    case 'investigation.execute': {
      if (!investigationOrchestrator || !investigationService) {
        throw new Error('Investigation orchestrator not initialized');
      }
      const result = await investigationOrchestrator.executeInvestigation(event.payload.investigationId);
      const investigation = investigationService.get(event.payload.investigationId);
      if (investigation) {
        sendToRenderer({ type: 'investigation.updated', payload: { investigation } });
      }
      sendToRenderer({
        type: 'investigation.plan',
        payload: { investigationId: event.payload.investigationId, plan: result.plan },
      });
      return result;
    }
    ...
    case 'investigation.pauseTask': {
      if (!investigationOrchestrator) throw new Error('Investigation orchestrator not initialized');
      investigationOrchestrator.pauseTask(event.payload.plannedTaskId, event.payload.reason);
      return { success: true };
    }

    case 'investigation.resumeTask': {
      if (!investigationOrchestrator) throw new Error('Investigation orchestrator not initialized');
      investigationOrchestrator.resumeTask(event.payload.plannedTaskId);
      return { success: true };
    }

    case 'investigation.reprioritizeTask': {
      if (!investigationOrchestrator) throw new Error('Investigation orchestrator not initialized');
      investigationOrchestrator.reprioritizeTask(
        event.payload.plannedTaskId,
    ...
    case 'investigation.createTask': {
      ...
      investigationOrchestrator.createHumanDirectedTask(event.payload.investigationId, event.payload.task);
      const investigation = investigationService.get(event.payload.investigationId);
      if (investigation) {
        sendToRenderer({ type: 'investigation.updated', payload: { investigation } });
      }
    ...
    case 'investigation.applyHumanInterruption': {
      if (!investigationOrchestrator) throw new Error('Investigation orchestrator not initialized');
      const result = investigationOrchestrator.applyHumanInterruption(event.payload.investigationId, event.payload);
      if (result.updatedInvestigation) {
        sendToRenderer({ type: 'investigation.updated', payload: { investigation: result.updatedInvestigation } });
      }
      return result;
    }
    ...
    case 'investigation.cancelTask': {
      if (!investigationOrchestrator) throw new Error('Investigation orchestrator not initialized');
      investigationOrchestrator.cancelTask(event.payload.plannedTaskId, event.payload.reason);
      return { success: true };
    }
    ...
    case 'investigation.getReport': {
      if (!investigationService) throw new Error('Investigation service not initialized');
      return investigationService.generateReport(event.payload.investigationId);
    }
    ...
    case 'investigation.exportReport': {
      ...
      if (!investigation) {
        return { success: false, error: `Investigation not found: ${investigationId}` };
      }
      const safeTitle = investigation.title.replace(/[^a-zA-Z0-9-_]+/g, '-').replace(/^-+|-+$/g, '') || investigation.id;
      const result = await dialog.showSaveDialog(mainWindow!, {
        title: 'Export Investigation Report',
        defaultPath: `investigation-report-${safeTitle}-${new Date().toISOString().split('T')[0]}.md`,
        filters: [
      ...
      try {
        ...
        logError('[Investigation] Error writing report:', writeError);
        return { success: false, error: writeError instanceof Error ? writeError.message : String(writeError) };
      }
    ...
    case 'synthetic.status': {
      if (!syntheticEnvironmentService) throw new Error('Synthetic environment service not initialized');
      return {
        available: syntheticEnvironmentService.isAvailable(),
        status: syntheticEnvironmentService.status(),
      };
    }
    ...
    case 'synthetic.loadScenario': {
      ...
      return {
        ...
        recordCounts: syntheticEnvironmentService.status().recordCounts,
      };
    }

    case 'synthetic.reset': {
      if (!syntheticEnvironmentService) throw new Error('Synthetic environment service not initialized');
      syntheticEnvironmentService.reset();
      return { success: true };
    }

    case 'synthetic.seedInvestigation': {
