...
export function WelcomeView() {
  ...
  const handleActionClick = (action: QuickAction) => {
    ...
    if (action.id !== selectedAction) {
      ...
      if (textareaRef.current) {
        textareaRef.current.value = action.prompt;
  ...
  const quickActions: QuickAction[] = [
    {
      id: 'investigate',
      label: t('welcome.actionInvestigate'),
      icon: Search,
      prompt: t('welcome.actionInvestigatePrompt'),
      color: '#38BDF8',
    },
    {
      id: 'analyze',
      label: t('welcome.actionAnalyze'),
      icon: BarChart3,
      prompt: t('welcome.actionAnalyzePrompt'),
      color: '#8B7CFF',
    },
    {
      id: 'scan',
      label: t('welcome.actionScan'),
      icon: Scan,
      prompt: t('welcome.actionScanPrompt'),
      color: '#60A5FA',
    },
    {
      id: 'hunt',
      label: t('welcome.actionHunt'),
      icon: Shield,
      prompt: t('welcome.actionHuntPrompt'),
      color: '#F5B942',
    },
    {
      id: 'report',
      label: t('welcome.actionReport'),
      icon: FileText,
      prompt: t('welcome.actionReportPrompt'),
      color: '#34D399',
    },
  ];
      ...
      <div className="max-w-[840px] w-full space-y-7 animate-fade-in">
        <div className="space-y-4 text-center">
          <div className="flex items-center justify-center gap-4">
            <KinLogo className="w-12 h-12 md:w-14 md:h-14" />
            <div className="text-left">
              <h1 className="text-[2.1rem] md:text-[2.8rem] leading-none font-semibold tracking-[-0.05em] text-text-primary">
                KIN
              </h1>
              <p className="text-xs font-medium tracking-[0.12em] uppercase text-text-muted mt-1">
                {t('welcome.tagline')}
              </p>
            </div>
          </div>
          <p className="text-sm md:text-base text-text-secondary text-center max-w-lg mx-auto leading-relaxed">
            {t('welcome.subtitle')}
          </p>
        </div>
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm transition-all ${
                  isActive
                    ? 'border-accent/40 bg-accent-muted text-accent shadow-soft'
                    : 'border-border-subtle bg-background/65 text-text-secondary hover:bg-surface-hover hover:text-text-primary hover:border-border'
                }`}
              >
                <Icon
        ...
        <form
          ...
          onDrop={handleDrop}
          className={`rounded-2xl border bg-card/90 shadow-soft px-5 py-5 space-y-4 transition-all ${
            isDragging ? 'ring-2 ring-accent border-accent/40 bg-accent/5' : 'border-border hover:border-border-muted'
          }`}
        >
          {/* Image previews */}
          {pastedImages.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 pb-3 border-b border-border w-full">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-border">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-accent text-white hover:bg-accent-hover shadow-soft"
            >
